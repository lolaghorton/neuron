...
filepath = './Data_dendrite_properties.tsv'
...
def inspect_path(term_dend_of_path, na_threshold):
	...
	for c in p:
		data.append(np.loadtxt(f'./data/attenuation/EPSP/dendrite_{p[0]}_nstim_{na_threshold}/{compartment_names[c]}.dat'))
...
def visualize_compare_data(amp_data, data_apicals, data_basals, title_text='', range_std_mult=2, ID_trunk=ID_trunk, ID_atypical=ID_atypical, filter_trunk_atypical=True, filter_non_corresponding=True, assume_equal_variances=False):
	...
	print(f'{title_text} | T-test statistic: {stat:.04f} | p-value: {pval:.06f} ({"not " if not assume_equal_variances else ""}assuming equal variances) | {"NS" if pval>=0.05 else "*"}')
...
# ==== Passive (EPSP) attenuation during a validation protocol for EPSPs ====
...
if __name__ == '__main__':
	...
	(amplitudes, widths, delays, delays_dV) = util.pickle_load('./data/figure5/EPSP_validation_results.pickle')

	# We need to find out the number of stims required to elicit a <target> mV depolarization at the dendrite.
	# The <target> is set to 20 mV, but we can adjust it.
	# To do so, we will look in the "amplitudes" variable, which is <50,200,2> in shape - dendrites, nstims, <dend or soma>.

	dend_amps = amplitudes[:,:,0]
	target_dV = 20
	dend_amps = np.array(dend_amps)
	dend_diff = np.abs(dend_amps - target_dV)
	idx_dV = np.argmin(dend_diff, axis=1)  # This variable contains the appropriate indices (in axis #1, nstims) for all dendrites, in order to get close to 20 mV dV
	na_thresholds = idx_dV+1  # for compatibility with pre-existing code - CAUTION: this does not relate to actual Na spiking thresholds here, but EPSPs!!!  
	
	print(f'Proximity to selected voltage ({target_dV} mV):')
	actual_amps = np.array([dend_amps[i,x] for i,x in enumerate(na_thresholds-1)])
	actual_amps_mean = np.mean(actual_amps)
	actual_amps_std = np.std(actual_amps)
	print(f'\tMean and st.dev. of dendritic EPSP amplitude: {actual_amps_mean:.04f}',end='')
	print(u"\u00B1", end='')  #plus/minus symbol
	print(f'{actual_amps_std:.04f}')
	...
	try:	
		amp_data = util.pickle_load('./data/figure5/amp_data_EPSPs.pickle')
	except:
		print(f'Processed EPSP attenuation data missing from ./Data/F5')
		raise Exception
	...
	if override_filtering_forDistance:
		fig = visualize_compare_data(amp_data, data_apicals, data_basals, title_text='Supp.Fig.8C (Attenuation vs distance from the soma)',range_std_mult=range_std_mult,filter_trunk_atypical=False, filter_non_corresponding=False, assume_equal_variances=assume_equal_variances)
	else:
		fig = visualize_compare_data(amp_data, data_apicals, data_basals, title_text='Supp.Fig.8C (Attenuation vs distance from the soma)',range_std_mult=range_std_mult,filter_trunk_atypical=filter_trunk_atypical, filter_non_corresponding=filter_non_corresponding, assume_equal_variances=assume_equal_variances)
	...
	fig = visualize_compare_data(amp_data, data_apicals, data_basals, title_text='Supp.Fig.8D (Attenuation vs. branch order)',range_std_mult=range_std_mult,filter_trunk_atypical=False, filter_non_corresponding=False, assume_equal_variances=assume_equal_variances)
	...
	fig = visualize_compare_data(amp_data, data_apicals, data_basals, title_text='Figure 5A (Attenuation vs length)',range_std_mult=range_std_mult,filter_trunk_atypical=filter_trunk_atypical, filter_non_corresponding=filter_non_corresponding, assume_equal_variances=assume_equal_variances)
	...
	fig = visualize_compare_data(amp_data, data_apicals, data_basals, title_text='Figure 5B (Attenuation vs diameter)',range_std_mult=range_std_mult,filter_trunk_atypical=filter_trunk_atypical, filter_non_corresponding=filter_non_corresponding, assume_equal_variances=assume_equal_variances)
